...
model = dict(
    ...
    pretrained='pre_trained/pre_trained_with_split_img.pth',
    interleaved=True,
    num_stages=3,
    backbone=dict(
        type='ResNeXt',
...
test_cfg = dict(
    rpn=dict(
        ...
        nms_thr=0.7,
        min_bbox_size=0),
    rcnn=dict(score_thr=0.06, nms=dict(type='nms', iou_thr=0.3), max_per_img=500),
...
train_pipeline = [
    ...
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels', 'template']),
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(
	...
	img_scale=(1536, 636), #0.5183
        ...
        transforms=[
            ...
            dict(type='Normalize', **img_norm_cfg),
            dict(type='Pad', size_divisor=32),
            dict(type='ImageToTensor', keys=['img', 'template']),
            dict(type='Collect', keys=['img', 'template']),
        ])
]
data = dict(
    imgs_per_gpu=1,
...
log_level = 'INFO'
work_dir = './work_dirs/get_split_window_with_normal'
load_from = None
resume_from = None
workflow = [('train', 1)]
