...
class ToDataContainer(object):
    ...
    def __init__(self, fields=(dict(key='img', stack=True), dict(key='gt_bboxes'), dict(key='gt_labels'), dict(key='template_img'))):
        self.fields = fields
...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        if 'img' in results:
            ...
            template = np.ascontiguousarray(results['template'].transpose(2, 0, 1))
            results['template'] = DC(to_tensor(template), stack=True)
