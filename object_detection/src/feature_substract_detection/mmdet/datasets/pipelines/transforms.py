...
class Resize(object):
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            template, scale_factor = mmcv.imrescale(results['template'], results['scale'], return_scale=True)
        else:
            img, w_scale, h_scale = mmcv.imresize(results['img'], results['scale'], return_scale=True)
            template, w_scale, h_scale = mmcv.imresize(results['template'], results['scale'], return_scale=True)
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            results['template'] = mmcv.imflip(results['template'])
            # flip bboxes
            for key in results.get('bbox_fields', []):
...
class Pad(object):
    ...
    def _pad_img(self, results):
        if self.size is not None:
            ...
            padded_template = mmcv.impad(results['template'], self.size)
        elif self.size_divisor is not None:
            padded_img = mmcv.impad_to_multiple(results['img'], self.size_divisor, pad_val=self.pad_val)
            padded_template = mmcv.impad_to_multiple(results['template'], self.size_divisor, pad_val=self.pad_val)
        results['img'] = padded_img
        results['template'] = padded_template
...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        results['template'] = mmcv.imnormalize(results['template'], self.mean, self.std, self.to_rgb)
...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        template_img = template_img[crop_y1:crop_y2, crop_x1:crop_x2, :]
        img_shape = img.shape
        results['img'] = img
...
class PhotoMetricDistortion(object):
    ...
    def __call__(self, results):
        ...
        if random.randint(2):
            ...
            template_img[..., 1] *= random.uniform(self.saturation_lower, self.saturation_upper)
        ...
        if random.randint(2):
            ...
            img[..., 0][img[..., 0] > 360] -= 360
            img[..., 0][img[..., 0] < 0] += 360
            template_img[..., 0] += random.uniform(-self.hue_delta, self.hue_delta)
...
class Expand(object):
    ...
    def __call__(self, results):
        ...
        print('expand image')
        i += 1
        img, template_img, boxes = [results[k] for k in ('img', 'template_img', 'gt_bboxes')]

        h, w, c = img.shape
        ratio = random.uniform(self.min_ratio, self.max_ratio)
        expand_img = np.full((int(h * ratio), int(w * ratio), c), self.mean).astype(img.dtype)
        expand_template_img = np.full((int(h * ratio), int(w * ratio), c), self.mean).astype(template_img.dtype)
        left = int(random.uniform(0, w * ratio - w))
        top = int(random.uniform(0, h * ratio - h))
        expand_img[top:top + h, left:left + w] = img
        expand_template_img[top:top + h, left:left + w] = template_img
        boxes = boxes + np.tile((left, top), 2).astype(boxes.dtype)

        results['img'] = expand_img
        results['template_img'] = expand_template_img
...
class MinIoURandomCrop(object):
    ...
    def __call__(self, results):
        print('min iou random crop')
        i += 1
        img, template_img, boxes, labels = [results[k] for k in ('img', 'template_img', 'gt_bboxes', 'gt_labels')]
        h, w, c = img.shape
        while True:
            ...
            for i in range(50):
                ...
                template_img = template_img[patch[1]:patch[3], patch[0]:patch[2]]
...
class Corrupt(object):
    ...
    def __call__(self, results):
        ...
        results['template_img'] = corrupt(results['template_img'].astype(np.uint8), corruption_name=self.corruption, severity=self.severity)
        return results
